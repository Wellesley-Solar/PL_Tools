...
class PLspec:
    ...
    def __init__(self, file, xy_name=["Wavelength", "Intensity"], mod="Z", timestamp="Exposure started time stamp"):
        ...
        Args:
            ...
            xy_name: array like, the zeroth element specifies the name of the x-data column,
            and the first element specifies the name of the y-data column
            mod: str, mode for background removal; "Z" for Zhangfit, "I" for Polynomial Fit
            timestamp: str, name of the column to be used as time stamp for frames
        ...
        """
        self.file = file
        self.og = pd.read_csv(self.file, index_col=None, header=0).rename(columns={xy_name[0]: "W",
                                                                                   xy_name[1]: "I",
                                                                                   timestamp: "tt"})
    ...
    def format_data(self):
        ...
        # Weighted Variance
        self.IstdW = np.sqrt(
            sum(self.df["I"]*(self.df["W"] - self.IweightedW)**2)/sum(self.df["I"]))
        self.IstdE = np.sqrt(
            sum(self.df["I"]*(self.df["E"] - self.IweightedE)**2)/sum(self.df["I"]))
        self.IskewW = sum(
            self.df["I"]*(self.df["W"] - self.IweightedW)**3)/sum(self.df["I"])
        self.IskewE = sum(
            self.df["I"]*(self.df["E"] - self.IweightedE)**3)/sum(self.df["I"])
        self.t_alt = self.og.iloc[0].tt/1e6  # +0.02
    ...
    def plot(self, mode="E"):
        ...
        Args:
            mode: str, the y-axis of the plot is always intensity, choosing the x-axis to
    ...
    def narrow(self, Wmin=550, Wmax=1000):
        """Enact the cutoff for the spectrum data and retain only what is in between Wmin and Wmax
...
class PLevol:
    def __init__(self, folder, t, xy_name=["Wavelength", "Intensity"], bl_mod="Z", timestamp="Exposure started time stamp"):
        ...
        Args:
            ...
            t: array like, a manually generated time series
            xy_name: array like, the zeroth element specifies the name of the x-data column,
            and the first element specifies the name of the y-data column
            bl_mod: str, mode for background removal; "Z" for Zhangfit, "I" for Polynomial Fit
            timestamp: str, name of the column to be used as time stamp for frames
        ...
        """
        self.og = [PLspec(file, xy_name=xy_name, mod=bl_mod, timestamp=timestamp)
                   for file in sorted(glob.glob(folder+"/*.csv"))]
        self.PLs = self.og
        self.t = t
        self.df = self.format_data()
        i_shift = np.where(self.df.t_alt.diff() < 0)[0]
        for i in i_shift:
            self.df["t_alt"] = np.concatenate(
                (self.df.iloc[:i].t_alt.values, self.df.iloc[i:].t_alt.values+self.df.iloc[i-1].t_alt))
        self.df["t"] = self.df["t_alt"].values
        self.t = self.df["t_alt"].values
        self.S = np.matrix([each.df["I"].values for each in self.PLs])

    def format_data(self):
        """ Format and compile key attributes of each frame into a data frame
        ...
        return pd.DataFrame(
            {
                "t": np.array(self.t),
                "t_alt": np.array([each.t_alt for each in self.PLs]),
    ...
    def ridge(self, n=5, fset=0.15, alpha=0.3, startIndex=0):
        ...
        fig, ax = plt.subplots()
        i_frames = startIndex + \
            np.linspace(0, len(self.df_alt.columns)-1, n, endpoint=False,)
        i_frames = i_frames[::-1]
        for i, v in enumerate(i_frames):
            # ii = len(self.df_alt.columns)-2-int(v)
            #     print(v)
            ax.plot(self.df_alt.E, self.df_alt[str(
                int(v))]/max(self.df_alt[str(int(v))])-fset*i, "")
            ax.fill_between(
                self.df_alt.E, self.df_alt[str(
                    int(v))]/max(self.df_alt[str(int(v))])-fset*i,
                -fset*i,
                alpha=alpha,
        ...
        for i in range(self.C_nm.shape[0]):
            for j in range(self.C_nm.shape[1]):
                if i == j:
                    self.C_nm[i, j] = 0
                    continue
                self.C_nm[i, j] = self.numer[i, j] / \
                    (self.covar_mat[i, i]*self.covar_mat[j, j])

        self.sigma_n = float(
            np.median(self.noise_std_est(self.S, self.mat_bar)))
...
class Fitter():
    ...
    def common_param_set(self, mdl, peak, y_max):
        ...
        mdl.set_param_hint(
            'center', min=peak[0]-self.wiggle, max=peak[0]+self.wiggle)
    ...
    def my_modelgen(self, spec, peak_indicies):
        ...
        composite_model, params, x, y, x_min, x_max, x_range, y_max = self.basicparam(
            spec)
        ...
        for (i, basis_func), peak_index in zip(enumerate(spec['model']), peak_indicies):
            ...
            if basis_func['type'] in ['GaussianModel', 'LorentzianModel', 'VoigtModel', 'PseudoVoigtModel']:
                model.set_param_hint(
                    'sigma', min=self.bd_l_sigma, max=self.bd_u_sigma)
                model.set_param_hint(
                    'center', min=x[peak_index]-self.wiggle, max=x[peak_index]+self.wiggle)
                ...
                default_params = {
                    ...
                    # x_range / len(x) * np.min(peak_widths)
                    prefix+'sigma': 50
                }
            else:
            ...
            composite_model, params = self.model_params_update(
                composite_model, model, params, model_params)
        ...
        for (i, basis_func), peak in zip(enumerate(spec['model']), peaks):
            ...
            if basis_func['type'] in ['GaussianModel', 'LorentzianModel', 'VoigtModel', 'PseudoVoigtModel']:
                ...
                default_params = {
                    ...
                    # x_range / len(x) * np.min(peak_widths)#peak[2]
                    prefix+'sigma': peak[2]
                }
            else:
                raise NotImplemented(
        ...
        for (i, basis_func), peak in zip(enumerate(spec['model']), peaks):
            ...
            if basis_func['type'] in ['GaussianModel', 'LorentzianModel', 'VoigtModel', 'PseudoVoigtModel']:
                ...
                default_params = {
                    ...
                    prefix+'height': peak[1],
                    # peak[2]
                    prefix+'sigma': x_range / len(x) * np.min(self.peak_widths)
                }
            else:
                raise NotImplemented(
                    f'model {basis_func["type"]} not implemented yet')
    ...
    def fit(self, nframe=5, startIndex=2, zero_bg=True, verbose=True, mthd="lbfgsb", neval=10):
        ...
        for i in np.arange(0, nframe-startIndex)+startIndex:
            ...
            if self.zero_bg:
                y = np.array(
                    self.PLevol_obj.PLs[index].df["I"] - self.PLevol_obj.PLs[0].df["I"])
            else:
            ...
            if self.first_pass:
                ...
                output = composite_model.fit(
                    y, params, x=x, method=mthd, max_nfex=neval)
            else:
                peaks_found, pk_model, pk_params, lt_model, lt_params = self.both2spec(
                    x, y)
                out1 = pk_model.fit(
                    y, pk_params, x=x, method=mthd, max_nfex=neval, ftol=self.ftol)
                out2 = lt_model.fit(
                    y, lt_params, x=x, method=mthd, max_nfex=neval, ftol=self.ftol)
                ...
                if out1.chisqr <= out2.chisqr:
                    ...
                    output = out2
                if verbose:
                    print("Frame", index, "CWT Chi-sqr", out1.chisqr,
                          "\t Last  Chi-sqr", out2.chisqr)
            ...
            if verbose:
                ...
                if not self.first_pass:
                    ...
                    ax[1, 1].legend()
                    ax[1, 1].set_title(
                        "Peak Fitting with Last Fitting Parameters")
                else:
                    for peak in peaks_found:
                        ax[0, 0].axvline(x=x[peak], c='black',
                                         linestyle='dotted')
                ...
                ax[0, 1].plot(x, 0*np.arange(len(x)),
                              'k-', label="sum", alpha=0.3)
                ax[0, 1].plot(x, y-accu_comps, 'ro', label="sum")
                ax[0, 1].plot(x, y-accu_comps, 'b-', label="sum", alpha=0.1)
                # ax[0,1].legend()
                ax[0, 1].set_title(
                    "Residual " + str(sum(np.multiply(y-accu_comps, y-accu_comps))))
                # plt.close()
                ...
                for name, param in output.params.items():
                    ...
                    if "center" in name:
                        if param.stderr == None:
                            print('{:7s}    mean:{:8.5f}'.format(
                                name, param.value))
                            continue
                        print('{:7s}    mean:{:8.5f}    std: {:8.5f}'.format(
                            name, param.value, param.stderr))

            self.output_series.append(output)
            # other than the first iteration, use both2spec to find the best fit
            self.first_pass = False
            self.param_num.append(len(output.params))
        self.post_process()
        avgSeriesW = np.array([self.peak_avg(each)
                               for each in self.output_series])
        ...
        self.df_peaks = pd.DataFrame({
            "t": np.array(self.PLevol_obj.df["t"])[startIndex:nframe],
            "min_w": np.array(self.min_w)[:, 0],
            "maxe_int": np.array(self.min_w)[:, 1],
            "max_w": np.array(self.max_w)[:, 0],
            "mine_int": np.array(self.max_w)[:, 1],
            "max_e": Wavelen2eV(np.array(self.min_w)[:, 0]),
            "min_e": Wavelen2eV(np.array(self.max_w)[:, 0]),
        })
    ...
    def post_process(self):
        ...
        #             print(i, v, params[v].value,params[v].stderr)
                    tmp_ctr.append(
                        [param.value, output.params[name[:3]+'height'].value])
                    # tmp_ht.append()
            tmp_ctr.sort()
        ...
        for each in out.params:
            if each.find("center") != -1:
                ...
                denom += out.params[each[:3]+'height'].value * \
                    out.params[each[:3]+'sigma'].value
                numer += out.params[each].value*out.params[each[:3] +
                                                           'height'].value*out.params[each[:3]+'sigma'].value
        return numer/denom
